...
def Train(l_rate, Lambda):
    W = {
        ...
        'layer_2': tf.Variable(
            tf.random_normal([n_hidden_1, n_hidden_2], mean=0.0, stddev=3.0 / (n_input + n_hidden_1))),
        'layer_3': tf.Variable(
            tf.random_normal([n_hidden_2, n_hidden_3], mean=0.0, stddev=3.0 / (n_input + n_hidden_1))),
        'layer_4': tf.Variable(
            tf.random_normal([n_hidden_3, n_hidden_4], mean=0.0, stddev=3.0 / (n_input + n_hidden_1))),
        'output': tf.Variable(
            tf.random_normal([n_hidden_4, n_classes], mean=0.0, stddev=3.0 / (n_hidden_1 + n_classes)))
    }
    b = {
        'Layer_1': tf.Variable(tf.random_normal([n_hidden_1])),
        'Layer_2': tf.Variable(tf.random_normal([n_hidden_2])),
        'Layer_3': tf.Variable(tf.random_normal([n_hidden_3])),
        'Layer_4': tf.Variable(tf.random_normal([n_hidden_4])),
    ...
    y_predicted_2 = tf.nn.relu(tf.add(tf.matmul(y_predicted_1, W['layer_2']), b['Layer_2']))
    y_predicted_3 = tf.nn.relu(tf.add(tf.matmul(y_predicted_2, W['layer_3']), b['Layer_3']))
    y_predicted_4 = tf.nn.relu(tf.add(tf.matmul(y_predicted_3, W['layer_4']), b['Layer_4']))
    y_predicted = tf.matmul(y_predicted_4, W['output']) + b['output']
    # Error definition
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_target, logits=y_predicted)
           ...
           + tf.reduce_sum(tf.square(W['layer_2'])) * Lambda / 2 + tf.reduce_sum(
        tf.square(W['layer_3'])) * Lambda / 2 + tf.reduce_sum(tf.square(W['layer_4'])) * Lambda / 2
...
def run(lr, lam, batch_size, num_epoch):
    ...
    while num_update < num_epoch:
        ...
        for step in range(num_steps):
            ...
            err_valid, y_prediction_valid = sess.run([mse, y_predicted],
        ...
        if i == 5 or i == 10 or i == 15 or i == 20:
            save_path = saver.save(sess,
                                   "/home/horis/Documents/Machine Learning examples/Neural Networks/Feedforward_models/model" + str(
                                       i) + ".ckpt")
...
def BuildRandomData():
    learning_rate = np.math.exp(np.random.uniform(-7.5, -4.5))
    layer_number = np.random.choice(range(1, 6))
    number_of_units = np.random.choice(range(100, 501))
    lam = np.math.exp(np.random.uniform(-9, -6))
    print("Learnning rate:" + str(learning_rate))
    print("Number of layers:" + str(layer_number))
    print("Number of hidden units per layer:" + str(number_of_units))
    print("Weight Decay Coefficient:" + str(lam))
...
if __name__ == '__main__':
    ...
    # Parameters
    learning_rate = 0.008612953987540091
    training_epochs = 700
    batch_size = 500
    lam = 0.00031767162516536506

    # Network Parameters
    n_hidden_1 = 200  # 1st layer number of features
    n_hidden_2 = 200  # 1st layer number of features
    n_hidden_3 = 200  # 1st layer number of features
    n_hidden_4 = 200  # 1st layer number of features
