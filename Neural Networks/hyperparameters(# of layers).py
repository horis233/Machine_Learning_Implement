...
def Train(l_rate, Lambda):
    W = {
        ...
        'layer_2': tf.Variable(tf.random_normal([n_hidden_1, n_hidden_2], mean=0.0, stddev=3.0 / (n_hidden_1 + n_hidden_2))),
        'output': tf.Variable(tf.random_normal([n_hidden_2, n_classes], mean=0.0, stddev=3.0 / (n_hidden_2 + n_classes)))
    }
    b = {
    ...
    y_predicted = tf.matmul(y_predicted_2,W['output'])+b['output']
    # Error definition
