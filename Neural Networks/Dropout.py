...
def Train(l_rate, Lambda):
    ...
    y_predicted_1_dropout = tf.nn.dropout(y_predicted_1,keep_prob=0.5)
    y_predicted = tf.matmul(y_predicted_1_dropout, W['output'])+b['output']
    # Error definition
...
def run(lr, lam, batch_size, num_epoch):
    ...
    while num_update < num_epoch:
        ...
        if i==5 or i==10 or i==15 or i==20:
            save_path = saver.save(sess, "/home/horis/Documents/Machine Learning examples/Neural Networks/Dropout_models/model" + str(i) + ".ckpt")
...
def print_graph(lr, lam, batch_size, num_epoch):
    ...
    loss_training, loss_validation, _, acc, acc_validation, _ = run(lr, lam, batch_size, num_epoch)
    plt.figure()
